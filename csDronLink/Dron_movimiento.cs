...
namespace csDronLink
{
    public partial class Dron
    {
        ...
        private void _IrAPosicionLocal(float localX, float localY, float localZ, Action<byte, object> f = null, object param = null)
        {
            // Paramos el bucle de navegación (por si estuviera en marcha)
            this.navegando = false;
            var msg = new MAVLink.mavlink_set_position_target_local_ned_t
            {
                target_system = this.id, // ID del dron
                target_component = 1, // Componente (generalmente 1)
                coordinate_frame = (byte)MAV_FRAME.LOCAL_NED,
                type_mask = (ushort)(
               POSITION_TARGET_TYPEMASK.VX_IGNORE |
               POSITION_TARGET_TYPEMASK.VY_IGNORE |
               POSITION_TARGET_TYPEMASK.VZ_IGNORE |
               ...
               POSITION_TARGET_TYPEMASK.YAW_RATE_IGNORE
                ...
                x = localX, // metros Norte
                y = localY, // metros Este
                z = localZ, // metros (negativo = arriba)
                vx = 0,
                vy = 0,
                vz = 0,
                afx = 0,
            ...
            // Generar el paquete MAVLink
            byte[] packet = mavlink.GenerateMAVLinkPacket10(MAVLINK_MSG_ID.SET_POSITION_TARGET_LOCAL_NED, msg);
        ...
        public void IrAPosicionLocal(float x, float y, float z, Boolean bloquear = true, Action<byte, object> f = null, object param = null)
        {
            if (bloquear)
            {
                this._IrAPosicionLocal(x, y, -z);
            }
            else
            {
                Thread t = new Thread(() => _IrAPosicionLocal(x, y, -z, f, param));
                t.Start();
            }
        }
    }
